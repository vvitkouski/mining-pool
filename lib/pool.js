...
var pool = module.exports = function pool(config, logger){
	...
	var redisKeyTotalMinersConnections = "miners-connections-total";
	var redisKeyTotalWorkers = "miners-total";
    ...
    function SetupJobManager(){
        ...
        _this.jobManager.on('newJobs', function(templates){
            ...
            _this.redisClient.publish(topicNameShares,JSON.stringify({
                ...
                workerAddr: shareData.workerAddress,
                jobId: job.jobId,
    ...
    function StartStratumServer(){
        ...
        }).on('addConnectionFromIP', function(connectionNumFromIP){
				var totalConnections = 0;
				var totalWorkers = 0;
				for (var k in connectionNumFromIP) {
					totalConnections += 1;
					totalWorkers += connectionNumFromIP[k];
					logger.info('Connections from IP '+k+': '+connectionNumFromIP[k]+' =========================================================');
				}
				logger.info('New connection From IP. Total: '+totalConnections+' =========================================================');
				_this.redisClient.set(redisKeyTotalMinersConnections, totalConnections, _ => {});
				_this.redisClient.set(redisKeyTotalWorkers, totalWorkers, _ => {});
