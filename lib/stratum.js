...
var StratumServer = exports.Server = function StratumServer(config){
    ...
    this.addConnectionFromIP = function(ipAddress){
        if (config.whitelistIps && config.whitelistIps.includes(ipAddress)){
			_this.emit('addConnectionFromIP', _this.connectionNumFromIP);
            return true;
        }
    ...
    (function init(){
        ...
        } else if (config.pool.tls) {
            var tls = require('tls');
            var fs = require('fs');
            serverFactory = tls.createServer
            options.key = fs.readFileSync(config.pool.privateKeyPath)
            options.cert = fs.readFileSync(config.pool.publicCertPath)
        }

        _this.server = serverFactory(options, function(socket) {
